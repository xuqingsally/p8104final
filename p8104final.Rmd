...
lm_race<-lm(log_los~factor(race),data=df) 
lm_religion<-lm(log_los~factor(religion),data=df) 
lm_maritalstatus<-lm(log_los~factor(maritalstatus),data=df) 
lm_insurancetype<-lm(log_los~factor(insurancetype),data=df) 
...
lm_full<-lm(log_los~mews+is30dayreadmit+cindex+evisit+ageyear+temperature+heartrate+respirationrate+bpdiastolic+bpsystolic+o2sat,data=df)
...
## Bootstrapping 
```{r}
train <- sample(length(df_rm), length(df_rm)/2)
...
bias <- bootstrap_coeff - bootstrap_results$t0
...
newsummary(lm(log_los ~ is30dayreadmit + cindex + evisit + ageyear + temperature + heartrate + respirationrate + bpdiastolic + bpsystolic,df_rm))
#PRESS slighter than SSE
...
conf_ints <- data.frame(confint(lm_best))
conf_ints  <- paste(round(conf_ints [,1], digits = 4), 
                     round(conf_ints [,2], digits = 4), 
                     sep=", ")
model_table <- cbind(variable_names, coeff, std_err, conf_ints)
